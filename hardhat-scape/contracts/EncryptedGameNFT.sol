...
contract EncryptedGameNFT is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    function grantTemporaryAccess(
        ...
        game.authorizedUsers[user] = true;
        game.accessExpirations[user] = expirationTime;
    ...
    function _calculatePriceInETH(uint256 usdPriceCents) internal view returns (uint256) {
        ...
        uint256 priceInWei = (usdPriceCents * 10**18) / 
                            (uint256(uint64(ethPrice.price)) * 10**10);
    ...
    function _hasMetadataAccess(uint256 tokenId, address user) internal view returns (bool) {
        ...
        GameNFT storage game = gameNFTs[tokenId];
        if (game.authorizedUsers[user] && 
            block.timestamp <= game.accessExpirations[user]) {
            return true;
        }
    ...
    function _verifyOracleProof(bytes memory proof) internal view returns (bool) {
        // Decode proof and verify oracle signature
        TransferProof memory decodedProof = _decodeTransferProof(proof);
        
        // Check timestamp validity (within last hour)
        require(
            block.timestamp - decodedProof.timestamp <= 3600, 
            "Proof expired"
        );
        
        // Verify oracle signature
        bytes32 messageHash = keccak256(abi.encodePacked(
            decodedProof.oldDataHash,
            decodedProof.newDataHash,
            decodedProof.receiverPublicKey,
            decodedProof.timestamp
        ));
        
        address recoveredSigner = _recoverSigner(messageHash, decodedProof.oracleSignature);
        ...
        returns (TransferProof memory) 
    {
        ...
        assembly {
            let dataPtr := add(proof, 32)
            decodedProof := mload(dataPtr)
        }
        ...
        returns (string memory) 
    {
        ...
        return string(abi.encodePacked(
            ...
            '"creator":"', _addressToString(game.creator), '",',
            '"price_usd":', _uint256ToString(game.priceUSD), ',',
            '"encrypted":true,',
